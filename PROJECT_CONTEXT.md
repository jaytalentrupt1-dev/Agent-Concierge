...
- Reports module with role-scoped report list, CSV/XLSX/PDF/TXT/MD/DOCX/DOC import, local upload storage, per-report download/export, filtered CSV export, delete action, search/filter controls, and audit logs. The Reports View action opens an animated content-first preview modal backed by `GET /api/reports/{id}/preview`; CSV/XLSX files render actual stored rows in a scrollable table, PDFs embed from the stored file when the browser supports it, TXT/MD/DOCX files show extracted readable text, and missing/unsupported/legacy DOC previews show `Preview is not available. Please download the report.` instead of the generic backend-version message.
- Settings > Connectors for every logged-in role with per-user Gmail Email and WhatsApp connector cards, Google OAuth Gmail connection, test message, disconnect actions, and communication log tracking.
- Shared communication send flow for Vendors, Tickets, Expenses, Reports, and Settings test messages. Senders choose Email, WhatsApp, or Both, preview the message, then confirm before the backend writes communication logs. The send modal is channel-aware: Email shows recipient email and subject, WhatsApp shows recipient phone without email/subject, and Both shows email, phone, and subject with matching validation.
...
- Email/WhatsApp connectors default to mock mode when credentials are missing. Normal users connect Email through Google OAuth/Gmail only; SMTP/SendGrid remain backend compatibility paths but are no longer the normal Settings UI flow. Twilio WhatsApp and WhatsApp Cloud API service paths exist behind `.env` configuration only; no real credentials are committed. Other external systems remain mocked.
...
- Connector management is available for Email and WhatsApp for every logged-in user with a clean table-style layout: plug icon section header, communication log count badge with logs opener, connector table columns for Connector, Provider, Status, and Actions, purple Email icon, green WhatsApp icon, provider badges, status pills (`Connected`, `Not Connected`, or `Mock Mode`), and per-row gear action menus.
- Connector row gear menus include Connect / Configure, Send Test, View Details, and Disconnect. The bottom action row repeats Connect / Configure, Send Test, and Disconnect for the currently selected connector. Disconnect is disabled unless the connector is connected and asks for confirmation before disconnecting.
- Email connector rows show Provider and Gmail OAuth subtext. If Google OAuth is not configured, `Google email connection is not configured yet.` appears inside the Connectors section only, and clicking Connect / Configure shows the same inline connector error rather than a page-wide Settings alert. The View Details dialog shows provider, status, Gmail OAuth state, last connected, and last tested values.
- WhatsApp connector rows show Provider and mock/configuration subtext. View Details shows provider, status, WhatsApp provider/config state, last connected, and last tested values.
- Email connection for normal users is Google OAuth Gmail. `GET /api/connectors/google/start` creates a per-user OAuth state and returns the Google authorization URL, `GET /api/connectors/google/callback` exchanges the Google code and stores that user's Gmail connection, `POST /api/connectors/google/disconnect` disconnects the current user's Gmail, and `POST /api/connectors/google/test-email` sends a test through the current user's Email connector.
- Required Gmail OAuth environment values are `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI`, and `FRONTEND_URL`. If they are missing, Settings shows `Google email connection is not configured yet.` and local mock email mode remains available.
...
- Connector API responses sanitize Gmail token fields (`access_token`, `refresh_token`, `id_token`) and never expose them to the frontend. Legacy SMTP/SendGrid compatibility endpoints store only non-secret display metadata/boolean flags; WhatsApp real-mode secrets continue to come from `.env`/environment variables.
- Backend connector endpoints are `GET /api/connectors`, `GET /api/connectors/google/start`, `GET /api/connectors/google/callback`, `POST /api/connectors/google/disconnect`, `POST /api/connectors/google/test-email`, `POST /api/connectors/whatsapp/configure`, `POST /api/connectors/email/test`, `POST /api/connectors/whatsapp/test`, `POST /api/connectors/disconnect`, and `GET /api/communications/logs`.
- Connector endpoints identify the logged-in user from the bearer token for user-initiated actions and return/update only that user's connector rows. Google callback validates the OAuth `state` created by that user's authenticated start request before linking the Gmail account. Admin does not expose other users' connector secrets through these endpoints.
...
- When sending, the backend uses the current user's connector provider/configuration for that channel. If Gmail OAuth is configured but the current user has not connected Gmail, Email sends return `Please connect your email in Settings first.` If Google OAuth env values are missing, mock email remains available and sends are logged as `mock_sent`.
...
- `Edit Ticket` is enabled for users with ticket manager access. `Change Status` is hidden for Finance Manager on the Tickets page and the backend status endpoint rejects Finance Manager direct status updates; Admin and IT Manager keep their existing status-update access.
- Backend ticket role scope:
  - `admin` can view and manage all tickets.
  - `it_manager` can view/manage IT tickets only.
  - `finance_manager` can view/edit finance-related Admin tickets, such as invoice/payment/expense/reimbursement categories, but cannot directly update ticket status.
...
- Inventory import is flexible for compact files: at least one identifying column must be present (`Employee Name`, `Serial No.`, or `Model No.`), while `RAM`, `Disk`, `Location`, `Status`, and `Notes` are optional. Missing optional values preview/save as blank except `Status`, which defaults to `In Use`.
- Header matching accepts common variants including `employee`, `assigned to`, `user`, `name`, `Name's`, `serial number`, `Sr. No.`, `Sr No`, `asset serial`, `service tag`, `model number`, `Laptop Brand`, `"Laptop" Brand`, `Brand`, `memory`, `storage`, `hard disk`, `ssd`, `hdd`, `office location`, `branch`, `asset status`, `remarks`, `comment`, and `comments`.
- Real employee inventory spreadsheets with columns such as `Num`, `Name's`, `Sr. No.`, `"Laptop" Brand`, `RAM`, and `Location` map into Employee Name, Serial No., Model No., RAM, and Location. `Num` is ignored, Disk/Notes default blank, and Status defaults to `In Use`.
- Inventory import scans for the strongest recognizable header row instead of assuming the first non-empty row is the header, so title rows and report metadata above the real headers are ignored.
- Import validates only the available flexible shape or legacy shape and shows an import preview before saving.
...
- Import preview also shows detected column labels and the detected header row number before confirmation.
- The import preview table is horizontally scrollable so valid imported fields are visible before confirmation.
- If a file has no recognizable inventory columns or no identifying compact-inventory column, import preview shows `This file does not match the inventory template. Please download and use the sample template.` instead of flooding the user with row-level required-field errors.
...
- Conci AI uses the local/mock role-scoped action engine by default.
- If `AI_PROVIDER=deepinfra` and a real `DEEPINFRA_API_KEY` are configured, Conci AI uses DeepInfra's OpenAI-compatible chat completions endpoint at `https://api.deepinfra.com/v1/openai/chat/completions` for intent classification and answer refinement. The default model is `deepseek-ai/DeepSeek-V3`.
- DeepInfra receives only the user's message plus allowed intent ids for classification, and only already role-filtered answer/bullets for response cleanup. Unauthorized app rows are never sent to DeepInfra.
- If the DeepInfra key is missing, is a placeholder, or the API call fails, Conci AI falls back to the current local/mock behavior.
- The legacy planning mode is mock unless a real, non-placeholder `OPENAI_API_KEY` is provided.
...
- Focused Conci AI freshness test: `1 passed, 186 deselected in 0.54s`; it creates/updates ticket and vendor records and verifies Conci AI immediately reads the updated status/contact/phone/billing data.
- Backend tests: `187 passed in 28.15s` after expanding Dashboard Tasks by Status coverage across Open, In Progress, Completed, Waiting Approval, Cancelled, and Overdue.
- Frontend build: `npm run build` passed after adding Cancelled to the Tasks by Status frontend chart mapping; latest bundles include `dist/assets/index-BljT2VZH.css` and `dist/assets/index-DS_HPNDv.js`.
...
- Manually verify Conci AI freshness in the browser after creating or updating records in each module, especially while the Dashboard chat panel is already open.
...
- The Dashboard Conci AI panel renders backend follow-up questions, created record IDs, compact table responses, and confirmation buttons when an action requires confirmation. Confirmed action payloads are sent back to the backend, where role checks and validation are run again before anything is created.
...
- The Conci AI refresh icon reloads the latest role-scoped app context through the normal backend refresh flow, including dashboard, approvals, audit logs, tasks, tickets, notifications, vendors, expenses, travel/calendar, reports, inventory, and current user context where the user's role can access those modules. It shows a small loading/success/error notice such as `Context refreshed` without clearing chat messages, typed input, attachments, or session history. Clearing chat is a separate trash header action and requires confirming `Are you sure you want to clear this chat?`.
...
- `Conci AI` uses local rule-based responses from existing app data when external AI mode is not configured. If DeepInfra or OpenAI refinement is available, the backend may use that provider only to refine the already role-filtered answer and bullets; hidden unauthorized rows are never sent to the frontend or model.
- `Conci AI` now has a backend intent-understanding layer before answer generation. User input is normalized, common spelling mistakes are corrected, and local fuzzy/similarity matching maps alternate wording or typos to known intents such as date/time utility, calendar events, recent/open/my tickets, ticket history including past/resolved/closed tickets, ticket creation/status checks, pending approvals/my pending requests, open/my/overdue tasks, vendor counts/billing/details/requested counts, active/all vendors, inventory updates/in-use/submitted-to-vendor/summary, month-wise/category-wise/pending/last-month expenses, travel spend/recent travel records, reports, file reading, and help. Follow-up clarifications use recent chat topic context, so corrections like `I want earlier history` after a ticket-history request or `show more` after a vendor details response stay on the previous topic instead of jumping to an unrelated date/general answer. If DeepInfra or OpenAI classification is configured, the backend uses that provider first to classify the intent from the user message and allowed intent labels, never with unauthorized app rows; local high-priority guards override obvious external misclassifications such as `calendar events` being returned as a date question, and backend role checks still block unauthorized model-classified requests such as an Employee asking for vendor billing.
...
- Vendor and vendor-billing chatbot answers can include a compact table payload, and the Dashboard chat bubble renders those rows as a small theme-aware table inside the assistant message. Vendor detail questions such as food vendor details now return only one short sentence plus the vendor table with Vendor Name, Service, Contact, Phone, Billing, End Date, and Status; they do not include duplicate bullet summaries above the table. Vendor detail responses honor requested counts such as `26 vendors`; when no count/all/details request is present, the backend keeps a short first-page response and the frontend chat table supports horizontal and vertical scrolling inside the bubble so contact/phone/billing/status/end date remain readable. The Vendor Billing Dashboard modal's older local mini-chat now calls the same backend Conci AI endpoint instead of answering from already-loaded dashboard data, so its answers use the latest role-filtered vendor data too.
...
- Empty ticket/task/vendor/inventory/expense answers now use module-specific text such as `I could not find ticket history for your account.`, `You don’t have any open tickets right now.`, `You don’t have any open tasks right now.`, `No vendors found for that request.`, `No inventory items found for that request.`, and `No expenses found for that request.`
...
- CSV and `.xlsx` imports support flexible compact headers, default missing optional fields, show a preview, and persist rows only after Confirm Import.
...
- Files with no recognizable inventory columns or no identifying compact-inventory column show the template-mismatch message and keep Confirm Import disabled. Missing optional columns such as Notes, Status, Disk, RAM, or Location no longer block preview/import.
